function createUI(data: string[], { maxIndex = 0, index = 1, gap = 20, screenWidth = 300 }) {
  ...
  const counter =
    len > 1 ? `<div class="counter"><span class="num">${index + 1}</span><span> / </span><span>${len}</span></div>` : ''
  ...
  const content = `<div class="viewer-wrap" style="width:${(screenWidth + gap) * len - gap}px;">${items.join('')}</div>`
